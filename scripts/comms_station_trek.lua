                ...
                for n=1,player:getWaypointCount() do
                    addCommsReply("WP" .. n, function()
                        if player:takeReputationPoints(getServiceCost("reinforcements")) then
                            ...
                            -- ship type depends on faction!!
                            local faction = comms_target:getFaction()
                            local template = 'Adder MK5'
                            if faction == "Ferengi" then
                                template = "Ferengi Marauder"
                            elseif faction == "Starfleet" then
                                template = "Intrepid Class"
                            elseif faction == "Klingons" then
                                template = "Klingon Bird Of Prey"
                            elseif faction == "Romulans" then
                                template = "Romulan Bird Of Prey"
                            end

                            ship = CpuShip():setFactionId(comms_target:getFactionId()):setPosition(comms_target:getPosition()):setTemplate(template):setScanned(true):orderDefendLocation(player:getWaypoint(n))
